...
def create_folder(workspace_folder_path: str) -> str:
    """
    Args:
        workspace_folder_path(str): the path of the parent directory to create the folder in
    Returns:
        A string which is the path of the created folder with the name
        'SDLsplit_%Y%m%d+%H%M%S'
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_folder = os.path.join(workspace_folder_path, f"SDLsplit_{timestamp}")
